...
- API uses `FLOWBOT_DATABASE_URL`,
  ...
  `FLOWBOT_NOTIFICATION_ENVELOPE_KEY`.
- Workers use `WORKER_DATABASE_URL`, `FLOWBOT_WORKER_ENABLED=true`, both FlowBot
  envelope keys, and the approved email delivery configuration.
- Integration and notification keys must be independent base64-encoded 32-byte
  secrets. Rotate through the deployment secret manager, never tenant settings.
...
origin denials, and any provider-leak detector event.
