...
Claimed Flow graphs retain bounded `authoring` context beside the deterministic runtime nodes. The shared domain owns the exact six approved starting-journey factories and validates every generated graph before the tenant can save it. Flow Studio edits identity, bilingual greetings, brand, widget, lead/consent and handover context structurally, then submits the complete optimistic-revision snapshot through the existing draft boundary. Role permission allows discovery; active Flow authority separately gates every mutation, publication, installation and traffic command. Immutable versions retain the validated authoring context without allowing it to change runtime edges implicitly.
...
Claim materialization promotes the Builder's three role-behavior fields and current bilingual FAQs into typed playbook authority rather than leaving them as passive context. The authenticated Text and Voice Configuration surfaces edit those fields through the shared guided editor. The shared AI turn runtime used by Text and Voice selects only query-relevant FAQs, sends their locale-specific approved question and answer through the restricted policy boundary, and treats a matching FAQ as approved grounding without fabricating a knowledge citation. Missing or stale bilingual FAQ copy fails materialization; complete but unreviewed copy remains advisory.
