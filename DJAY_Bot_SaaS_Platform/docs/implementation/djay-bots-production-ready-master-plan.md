...
Builder handoff checkpoint, 2026-08-17: migration `0122_builder_flow_materialization.sql` and the tenant onboarding transaction now materialize a valid, tenant-claimed anonymous Flow graph into exactly one non-live `flow_bot` and editable `flow_draft`. Stable UUID mapping preserves messages, options, input capture, forms, handover, end nodes, bilingual copy, keywords and canvas positions; the Builder's underspecified card is conservatively converted to a message with an explicit warning. Invalid graphs remain durably claimed, record failed audit evidence and do not falsely create or publish a bot. Repeated onboarding is idempotent, an existing tenant Flow bot is never overwritten, and no version, deployment, entitlement or live traffic is created. Text/Voice claim materialization, complete templates and external acceptance remain open, so PR-06 and `ONB-004` remain only `in_progress`.
