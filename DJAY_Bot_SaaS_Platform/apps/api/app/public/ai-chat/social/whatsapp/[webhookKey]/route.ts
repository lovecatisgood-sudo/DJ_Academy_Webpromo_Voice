...
export async function GET(request: NextRequest, route: { params: Promise<{ webhookKey: string }> }) {
  ...
  const allowed = await enforceRateLimit(
    "ai_social_whatsapp_challenge", `${webhookKey.data}:${clientAddress(request)}`, 60, 60_000,
  );
  if (!allowed.allowed) return safeJson({ status: "rate_limited" }, 429);
  try {
    const connection = await services.aiSocialRuntime.connection(webhookKey.data, "whatsapp");
    if (!connection) return safeJson({ status: "not_found" }, 404);
    const challenge = verifySocialChallenge(
      "whatsapp", request.nextUrl.searchParams.get("hub.mode"),
...
export function POST(request: NextRequest, route: { params: Promise<{ webhookKey: string }> }) {
  return withWebhookAck("ai_chat", "whatsapp", () => handleWebhook(request, route));
}
...
async function handleWebhook(request: NextRequest, route: { params: Promise<{ webhookKey: string }> }) {
  ...
  const allowed = await enforceRateLimit(
    "ai_social_whatsapp_webhook", `${webhookKey.data}:${clientAddress(request)}`, 240, 60_000,
  );
  if (!allowed.allowed) return safeJson({ status: "rate_limited" }, 429);
  ...
  try {
    ...
    if (!verifySocialSignature(
      "whatsapp", rawBody, request.headers.get("x-hub-signature-256"), connection.credentials,
    )) return safeJson({ status: "signature_invalid" }, 401);

    const events = normalizeSocialWebhook("whatsapp", JSON.parse(new TextDecoder().decode(rawBody)) as unknown);
    const receipts = [];
    for (const event of events) {
      ...
      const receipt = await services.aiSocialRuntime.receive({
        webhookKey: webhookKey.data, channel: "whatsapp",
