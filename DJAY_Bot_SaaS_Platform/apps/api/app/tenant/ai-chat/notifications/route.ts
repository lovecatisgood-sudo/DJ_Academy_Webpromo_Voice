...
export async function GET(request: NextRequest) {
  ...
  return safeJson({ notifications: await resolved.services.tenantAiNotifications.list(resolved.context) });
}

export async function POST(request: NextRequest) {
  const resolved = await resolveTenantRequest(request);
  if (!resolved || !tenantRoleAllows(resolved.context.role, "ai_chat.deploy") || !(await hasTrustedOrigin(request))) {
    return safeJson({ status: "not_found" }, 404);
  }
  const key = resolved.services.aiNotificationEnvelopeKey;
  if (!key) return safeJson({ status: "not_available" }, 503);
  try {
    const result = await resolved.services.tenantAiNotifications.create(
