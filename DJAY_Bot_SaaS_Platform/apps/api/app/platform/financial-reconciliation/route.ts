...
export async function GET(request: NextRequest) {
  ...
  return safeJson({ financialReconciliation:
    await resolved.services.platformCommerce.financialReconciliationOverview(resolved.context) });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = input.operation === "request"
      ? await resolved.services.platformCommerce.requestFinancialReconciliationCase(resolved.context, input)
      : await resolved.services.platformCommerce.reviewFinancialReconciliationCase(resolved.context, input);
