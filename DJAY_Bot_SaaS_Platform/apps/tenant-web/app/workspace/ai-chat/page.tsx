...
type Deployment = { id: string; name: string; channel: string; keyPrefix: string | null; allowedOrigins: string[]; status: string; trafficStatus: "inactive" | "live"; livePlaybookVersionId: string | null; liveAt: string | null; createdAt: string };
...
export default function AiChatPage() {
  ...
  return <main className="workspace-shell"><WorkspaceSidebar active="ai_chat" workspaces={session.workspaces} selectedTenantId={session.selectedTenantId} onSelect={(id) => void session.selectWorkspace(id)} onLogout={() => void session.logout()} />
    <section id="workspace-main" className="workspace-main" tabIndex={-1}><WorkspaceSupportBanner tenantId={session.selectedTenantId} />
      <header className="workspace-header"><div><p>การสนทนาการขายที่อ้างอิงข้อมูลจริง</p><h1>{uiCopy("การตั้งค่า AI Text Bot", "AI Text Bot Configuration")}</h1></div><div className="deploy-command-row"><a className="secondary-link" href="/workspace">{uiCopy("กลับแดชบอร์ด", "Return to Dashboard")}</a><span className="role-label">{humanizePlanKey(capabilities?.planKey)} · {humanizeAccessMode(capabilities?.accessMode)}</span></div></header>
      {agents.length > 0 && !canEdit ? <section className="tool-band muted-band"><div className="band-heading"><div><p>{uiCopy("ยังไม่เปิดสิทธิ์ใช้งาน", "Access not active")}</p><h2>{uiCopy("ฉบับร่างของคุณถูกเก็บไว้อย่างปลอดภัย", "Your draft is preserved")}</h2></div><span>{uiCopy("อ่านอย่างเดียว", "Read only")}</span></div><p>{uiCopy("คุณเข้าดูการตั้งค่าได้ แต่ต้องเปิดใช้งานแพ็กเกจ AI Text ก่อนแก้ไข ทดสอบ เผยแพร่ หรือติดตั้ง", "You can review this configuration. Activate an AI Text package before editing, testing, publishing, or installing.")}</p><a className="secondary-link" href="/workspace/usage">{uiCopy("ดูแพ็กเกจและการใช้งาน", "View packages and usage")}</a></section> : null}
      ...
      {selectedAgent && draft && definition ? <>
        ...
        {canEdit ? <section className="tool-band muted-band"><div className="band-heading"><div><p>โหมดทดสอบที่ธุรกิจอนุญาต</p><h2>ดูตัวอย่างโดยไม่เปลี่ยนข้อมูล</h2></div><span>20 ครั้ง / นาที</span></div>
        ...
        <section className="tool-band"><div className="band-heading"><div><p>การส่งข้อมูลผู้สนใจที่ผ่านการคัดกรอง</p><h2>การแจ้งเตือนทางอีเมลธุรกิจ</h2></div><span>{notificationsLoadError ? "Unavailable" : `${notifications.length} active`}</span></div>
          {canEdit ? <form className="flowbot-deploy" onSubmit={createNotification}><label>ชื่อ<input name="name" minLength={2} maxLength={160} required /></label><label>อีเมลผู้รับ<input name="recipientEmail" type="email" maxLength={320} required /></label><button disabled={working || notificationsLoadError}>เพิ่มผู้รับ</button></form> : null}
        ...
        <section className="tool-band muted-band"><div className="band-heading"><div><p>ช่องทางพื้นฐาน</p><h2>การติดตั้งบนเว็บไซต์</h2></div><span>{deployments.length}{capabilities?.limits.deployments ? ` / ${capabilities.limits.deployments}` : ""}</span></div>
          {canEdit && selectedAgent.currentPublishedPlaybookVersionId ? <WebsiteDeploymentForm className="flowbot-deploy" onCreate={createDeployment} submitLabel="Create web deployment" working={working} /> : null}
          ...
          <div className="data-table">{deployments.map((item) => { const check = installChecks.find((candidate) => candidate.deploymentId === item.id); const updateAvailable = item.trafficStatus === "live" && item.livePlaybookVersionId !== selectedAgent.currentPublishedPlaybookVersionId; return <div className="data-row" key={item.id}><div><strong data-no-localize>{item.name}</strong><span data-no-localize>{item.allowedOrigins.join(", ")}</span></div><div><span>{uiCopy("ติดตั้ง", "Install")}: {check?.status || "not checked"}</span><span>{uiCopy("การใช้งานจริง", "Traffic")}: {item.trafficStatus}</span></div>{canEdit ? <div className="setup-action-row"><button type="button" className="secondary-command" disabled={working} onClick={() => void requestInstallCheck(item)}>{uiCopy("ตรวจสอบการติดตั้ง", "Check install")}</button>{item.trafficStatus === "live" ? <>{updateAvailable ? <button type="button" disabled={working} onClick={() => void changeTraffic(item, "go_live")}>{uiCopy("อัปเดตเวอร์ชันที่ใช้งานจริง", "Update live version")}</button> : null}<button type="button" className="secondary-command" disabled={working} onClick={() => void changeTraffic(item, "stop")}>{uiCopy("หยุดรับข้อความ", "Stop traffic")}</button></> : <button type="button" disabled={working || check?.status !== "verified"} onClick={() => void changeTraffic(item, "go_live")}>{uiCopy("เปิดใช้งานจริง", "Go live")}</button>}</div> : <code>{item.keyPrefix}...</code>}</div>; })}{!deployments.length ? <div className="pending-line"><strong>ยังไม่มีการติดตั้ง</strong><span>เผยแพร่ก่อนสร้างการติดตั้งบนเว็บ</span></div> : null}</div>
        </section>
