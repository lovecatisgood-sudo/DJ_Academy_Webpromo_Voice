...
const addOnLabels: Record<string, string> = { additional_administrator: "Additional administrator", additional_workspace: "Additional workspace", starter_branding_removal: "Remove DJay Bots branding" };
const serviceLabels: Record<string, string> = { flow_starter_setup: "Starter Flow Setup", flow_advanced_design: "Advanced Flow Design", flow_complex_automation: "Complex Flow Automation", knowledge_base_setup: "Knowledge-Base Setup", ai_sales_configuration: "AI Sales Configuration", ai_advanced_sales_system: "Advanced AI Sales System", voice_agent_setup: "Voice Agent Setup", custom_voice_automation: "Custom Voice Automation", enterprise: "Enterprise solution" };
const productGuides = {
  flowbot: { tutorialKey: "flowbot.setup", title: "Flow Bot setup", href: "/workspace/setup" },
...
export default function OperationsPage() {
  ...
  if (session.loading || !session.selectedTenantId) return <main className="workspace-loading">กำลังโหลดบริการ...</main>;
  if (loadError) return <WorkspacePageLoadError active="operations" title="บริการและส่วนเสริม" resource="operations" workspaces={session.workspaces} selectedTenantId={session.selectedTenantId} onSelect={(id) => void session.selectWorkspace(id)} onLogout={() => void session.logout()} onRetry={() => void load()} />;
  ...
  return <main className="workspace-shell"><WorkspaceSidebar active="operations" workspaces={session.workspaces} selectedTenantId={session.selectedTenantId} onSelect={(id) => void session.selectWorkspace(id)} onLogout={() => void session.logout()} />
    <section id="workspace-main" className="workspace-main" tabIndex={-1}><header className="workspace-header"><div><p>การดำเนินงานเวิร์กสเปซ</p><h1>บริการและส่วนเสริม</h1></div><span className="role-label">{operations?.activeAddOns.length || 0} active add-ons</span></header>
      {message ? <p className="inline-message dashboard-inline-message" role="status">{message}</p> : null}
      {!canManage ? <WorkspaceViewOnly>คุณดูความคืบหน้าการตั้งค่าและบริการที่ใช้งานได้ ผู้ดูแลเวิร์กสเปซเป็นผู้ขอส่วนเสริมหรือบริการจากผู้เชี่ยวชาญ</WorkspaceViewOnly> : null}
      <section className="tool-band muted-band"><div className="band-heading"><div><p>การเริ่มใช้ผลิตภัณฑ์</p><h2>คู่มือการตั้งค่า</h2></div><span>อ้างอิงหลักฐานในหน้าภาพรวม</span></div>
        <p className="control-copy">ระบบคำนวณความคืบหน้าจากหลักฐานบนเซิร์ฟเวอร์ในหน้าภาพรวม ลิงก์เหล่านี้เปิดการตั้งค่าผลิตภัณฑ์จริง แต่ไม่ทำให้คู่มือเสร็จสิ้นโดยอัตโนมัติ</p>
        <div className="operations-guide-list">{subscriptions.map((subscription) => {
          const guide = productGuides[subscription.productKey];
          return <div key={subscription.id}><div><strong>{guide.title}</strong><span>{subscription.publicName}</span></div><a href={guide.href}>เปิดการตั้งค่า</a><a className="secondary-link" href="/workspace">ดูรายการตรวจสอบก่อนเปิดใช้ (View launch checklist)</a></div>;
        })}{!subscriptions.length ? <p className="field-help">คู่มือการตั้งค่าจะแสดงเมื่อเวิร์กสเปซนี้มีสิทธิ์ใช้ผลิตภัณฑ์</p> : null}</div>
      </section>
      <section className="tool-band"><div className="band-heading"><div><p>ความจุและรูปแบบการแสดงผล</p><h2>ขอส่วนเสริม</h2></div><span>จัดเตรียมหลังได้รับอนุมัติ</span></div>
        {canManage ? <><form className="operations-request-form" id="workspace-add-on-request" onSubmit={requestAddOn}><label>ส่วนเสริม<select name="addOnKey" value={selectedAddOn} onChange={(event) => setSelectedAddOn(event.target.value)}>{Object.entries(addOnLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
          <label>ข้อกำหนดผลิตภัณฑ์<select key={selectedAddOn} name="subscriptionId" defaultValue="" disabled={selectedAddOn === "additional_workspace"} required={selectedAddOn !== "additional_workspace"}><option value="">เลือกผลิตภัณฑ์</option>{eligibleSubscriptions.map((item) => <option key={item.id} value={item.id}>{item.publicName}</option>)}</select></label>
          <label>จำนวน<input name="quantity" type="number" min="1" max={selectedAddOn === "additional_workspace" || selectedAddOn === "starter_branding_removal" ? "1" : "100"} defaultValue="1" readOnly={selectedAddOn === "additional_workspace" || selectedAddOn === "starter_branding_removal"} required /></label><button disabled={working}>ส่งคำขอ</button></form>
        {selectedAddOn === "additional_workspace" ? <div className="workspace-request-fields"><label>ชื่อธุรกิจ<input name="workspaceBusinessName" form="workspace-add-on-request" minLength={2} maxLength={200} required /></label><label>ที่อยู่เวิร์กสเปซ<input name="workspaceSlug" form="workspace-add-on-request" pattern="[a-z0-9][a-z0-9-]{1,62}" minLength={2} maxLength={63} placeholder="second-business" required /></label></div> : null}</> : null}
        <div className="operations-status-list">{operations?.addOns.map((item) => <div key={item.id}><strong>{addOnLabels[item.addOnKey] || humanizeToken(item.addOnKey)}</strong><span>{item.quantity} requested</span><small>{humanizeToken(item.status)}</small></div>)}</div>
      </section>
      <section className="tool-band muted-band"><div className="band-heading"><div><p>บริการดำเนินงานโดยผู้เชี่ยวชาญ</p><h2>บริการติดตั้งและบริการ Enterprise</h2></div><span>{operations?.engagements.length || 0} engagements</span></div>
        {canManage ? <form className="operations-service-form" onSubmit={requestService}><label>บริการ<select name="serviceKind" defaultValue="knowledge_base_setup">{Object.entries(serviceLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
          <label>ผลิตภัณฑ์<select name="productKey" defaultValue=""><option value="">ทั้งเวิร์กสเปซ / Enterprise</option><option value="flowbot">FlowBot</option><option value="ai_chat">บอตข้อความ AI</option><option value="voice">บอตเสียง AI</option></select></label>
          <label className="wide-field">ความต้องการทางธุรกิจ<textarea name="brief" minLength={20} maxLength={10000} rows={4} required /></label><button disabled={working}>ขอนัดปรึกษา</button></form> : null}
        <div className="operations-engagement-list">{operations?.engagements.map((item) => <article key={item.id}><header><div><strong data-no-localize>{item.title}</strong><span data-no-localize>{item.scopeText}</span></div><div><small>{humanizeToken(item.status)}</small><span>ขั้นตอนถัดไป: {humanizeToken(item.nextActionOwner)}</span></div></header>
          <div className="engagement-timeline">{operations.engagementUpdates.filter((update) => update.engagementId === item.id).map((update) => <div key={update.id}><strong>{update.authorKind === "djai" ? "DJAI" : "ทีมของคุณ"}</strong><p data-no-localize>{update.body}</p><time dateTime={update.createdAt}>{new Date(update.createdAt).toLocaleString(currentIntlLocale())}</time></div>)}</div>
          {canManage && !["completed", "cancelled"].includes(item.status) ? <form onSubmit={(event) => void sendEngagementUpdate(event, item.id)}><label>ข้อมูลอัปเดตสำหรับทีมดำเนินงาน<textarea name="body" minLength={2} maxLength={5000} rows={2} required /></label><button disabled={working}>ส่งข้อมูลอัปเดต</button></form> : null}
