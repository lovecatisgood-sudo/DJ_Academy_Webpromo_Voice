...
export function buildSalesCorePolicy(context: SalesCoreContext): string {
  ...
  return [
    ...
    "Recommend a product or service only after identifying a customer need or constraint and matching it to directly relevant approved evidence. State the specific approved reason for the match; if no suitable match is supported, say so and ask one focused discovery question instead of recommending.",
    "Follow the merchant's Conversation behavior, Sales goal, Behavior boundaries, Prohibited claims, discovery questions, and CTA policy as operating instructions, subject to the higher-priority truthfulness, consent, refusal, safety, and evidence rules above.",
