  ...
  it("issues opaque Gen1 grants, reserves atomically, reconnects, and settles exactly once", async () => {
    ...
    await adminClient!`
      INSERT INTO tenancy.voice_deployments (
        ...
        traffic_status, live_playbook_version_id, live_at, created_by_membership_id
      ) VALUES (
        ...
        90, 30, 'live', ${playbookVersionId}::uuid, now(), ${membershipId}::uuid
      )
    `;
    ...
    await adminClient!`
      INSERT INTO tenancy.voice_deployments (
        ...
        max_call_seconds, reconnect_window_seconds, traffic_status, live_playbook_version_id,
        live_at, created_by_membership_id
      ) VALUES (
        ...
        'This is our automated voice assistant.', 90, 30, 'live', ${advancedPlaybookVersionId}::uuid, now(),
        'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbb11'::uuid
      )
    `;
    await expect(runtime.issue({
