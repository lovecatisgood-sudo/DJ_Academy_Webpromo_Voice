  ...
  async createAgent(context: TenantContext, input: Readonly<{ name: string; businessName: string; defaultLanguage: "th" | "en" }>) {
    return withTenantTransaction(this.client, context, async ({ sql }) => {
      ...
      const definition: AiPlaybook = {
        ...
        behaviorInstructions: "Ask focused questions, answer from approved information, and offer one useful next step.",
        behaviorBoundaries: "Do not invent facts or claim that an external action succeeded.",
        approvedFaqs: [],
