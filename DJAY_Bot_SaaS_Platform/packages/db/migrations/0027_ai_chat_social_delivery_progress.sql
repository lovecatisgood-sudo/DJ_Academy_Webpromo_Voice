ALTER TABLE tenancy.ai_social_outbound_deliveries
  ADD COLUMN delivered_part_count integer NOT NULL DEFAULT 0
  CHECK (delivered_part_count >= 0 AND delivered_part_count <= 100);
...
BEGIN
  ...
  IF fee_classification NOT IN ('reply', 'push', 'service_window_reply')
     ...
     OR completed_part_count < 0 OR completed_part_count > attempted_quantity
     OR cardinality(COALESCE(target_external_message_ids, '{}')) > completed_part_count
     OR EXISTS (
       SELECT 1 FROM unnest(COALESCE(target_external_message_ids, '{}')) external_id
       WHERE char_length(external_id) NOT BETWEEN 1 AND 500
     )
     OR (delivered AND (attempted_quantity = 0 OR completed_part_count <> attempted_quantity)) THEN
    RAISE EXCEPTION 'invalid_ai_social_delivery_result';
  END IF;
  SELECT delivery.tenant_id, delivery.connection_id, delivery.channel,
         delivery.message_id, delivery.attempt_count, delivery.delivered_part_count
  INTO runtime
  ...
  IF runtime.delivered_part_count + completed_part_count > 100
     OR (runtime.channel = 'line' AND fee_classification NOT IN ('reply', 'push'))
  ...
  SET status = CASE WHEN delivered THEN 'succeeded'
      ...
      locked_at = NULL,
      external_message_ids = delivery.external_message_ids || COALESCE(target_external_message_ids, '{}'),
      delivered_part_count = delivery.delivered_part_count + completed_part_count,
  ...
  IF changed = 1 AND NOT delivered THEN
    ...
    VALUES (runtime.tenant_id, 'ai_chat.social_delivery_failed', 'ai_social_delivery',
      ...
      'failed', jsonb_build_object(
        ...
        'completedPartCount', completed_part_count
      ));
  END IF;
  RETURN changed = 1;
END
...
REVOKE ALL ON FUNCTION tenancy.finish_ai_social_delivery_parts(uuid, boolean, text[], text, integer, integer, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION tenancy.claim_ai_social_delivery(timestamptz, timestamptz) TO djay_worker;
GRANT EXECUTE ON FUNCTION tenancy.finish_ai_social_delivery_parts(uuid, boolean, text[], text, integer, integer, text, boolean) TO djay_worker;
