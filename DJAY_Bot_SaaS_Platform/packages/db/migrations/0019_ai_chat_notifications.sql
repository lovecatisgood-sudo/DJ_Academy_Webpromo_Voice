CREATE OR REPLACE FUNCTION tenancy.claim_ai_chat_notification(
...
BEGIN
  IF session_user <> 'djay_worker'
     OR current_setting('app.service', true) IS DISTINCT FROM 'ai_chat_notification_worker' THEN
    RAISE EXCEPTION 'AI chat notification worker context required';
  END IF;
  RETURN QUERY
  WITH candidate AS (
    SELECT candidate_outbox.id
    FROM tenancy.outbox candidate_outbox
    WHERE candidate_outbox.topic = 'ai_chat.merchant_email.requested'
  ...
  SELECT claimed.id,
         ...
         COALESCE(
           ...
           AND claimed.payload->>'templateKey' = 'ai_chat.lead_qualified'
           AND claimed.payload->>'templateKey' = ANY(profile.allowed_template_keys)
           AND EXISTS (
             ...
             WHERE snapshot.tenant_id = claimed.tenant_id AND snapshot.product_key = 'ai_chat'
...
CREATE OR REPLACE FUNCTION tenancy.finish_ai_chat_notification(
...
SET search_path = pg_catalog, tenancy
AS $$
DECLARE changed integer; target_tenant_id uuid;
BEGIN
  IF session_user <> 'djay_worker'
     OR current_setting('app.service', true) IS DISTINCT FROM 'ai_chat_notification_worker' THEN
    RAISE EXCEPTION 'AI chat notification worker context required';
  ...
  WHERE target.id = target_outbox_id
    AND target.topic = 'ai_chat.merchant_email.requested'
  ...
  IF changed = 1 AND NOT delivered THEN
    INSERT INTO tenancy.audit_logs (
      ...
      target_tenant_id, 'ai_chat.notification_delivery_failed', 'tenant_outbox',
      target_outbox_id::text, COALESCE(current_setting('app.request_id', true), 'ai-chat-notification-worker'),
...
REVOKE ALL ON FUNCTION tenancy.claim_ai_chat_notification(timestamptz, timestamptz) FROM PUBLIC;
REVOKE ALL ON FUNCTION tenancy.finish_ai_chat_notification(uuid, boolean, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION tenancy.claim_ai_chat_notification(timestamptz, timestamptz) TO djay_worker;
GRANT EXECUTE ON FUNCTION tenancy.finish_ai_chat_notification(uuid, boolean, text, boolean) TO djay_worker;
