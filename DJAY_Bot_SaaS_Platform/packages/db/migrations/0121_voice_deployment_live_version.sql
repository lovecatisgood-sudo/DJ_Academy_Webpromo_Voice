ALTER TABLE tenancy.voice_deployments ADD COLUMN live_playbook_version_id uuid;

UPDATE tenancy.voice_deployments deployment
...
ALTER TABLE tenancy.voice_deployments
  ADD CONSTRAINT tenancy_voice_deployment_live_playbook_fk
  ...
  ADD CONSTRAINT tenancy_voice_deployment_live_playbook_required
    CHECK (traffic_status <> 'live' OR live_playbook_version_id IS NOT NULL);
...
BEGIN
  ...
  SELECT deployment.*, snapshot.id AS snapshot_id, snapshot.access_mode,
         snapshot.resolved_json, plan.plan_key
  INTO resolved
  FROM tenancy.voice_deployments deployment
  JOIN tenancy.ai_agents agent ON agent.tenant_id = deployment.tenant_id
    AND agent.id = deployment.agent_id AND agent.status = 'active'
  ...
  VALUES (target_session_id, resolved.tenant_id, resolved.id, resolved.agent_id,
    resolved.live_playbook_version_id, target_contact_id, target_conversation_id,
...
REVOKE ALL ON FUNCTION tenancy.issue_voice_session_grant(
  bytea, bytea, text, uuid, uuid, uuid, timestamptz, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION tenancy.issue_voice_session_grant(
  bytea, bytea, text, uuid, uuid, uuid, timestamptz, text) TO djay_voice_runtime;
